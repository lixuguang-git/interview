...
def load_results() -> pd.DataFrame:
    ...
    if RESULTS_FILE.exists():
        ...
        # 确保新字段存在
        if "Answer_Details" not in df.columns:
            df["Answer_Details"] = ""
        # 统一类型为字符串，填充缺失
        df["Answer_Details"] = df["Answer_Details"].apply(
            lambda v: "" if pd.isna(v) else (json.dumps(v, ensure_ascii=False) if isinstance(v, (dict, list)) else str(v))
        )
        return df
    return pd.DataFrame(columns=["Timestamp", "Name", "Phone", "Total_Score", "Details", "Bank", "Answer_Details"])
...
def candidate_view():
    ...
    if st.button("提交", type="primary", use_container_width=True):
        ...
        for idx, (qid_key, choice) in enumerate(answer_list):
            if idx < len(questions_list):
                ...
                detailed_answers.append({
                    ...
                    "Selected_Text": row[f"Option_{choice}"],
                    "Score": int(score),
                    "Bank": bank,
                    "Options": {
                        "A": f"{row['Option_A']} ({row['Score_A']}分)",
                        "B": f"{row['Option_B']} ({row['Score_B']}分)",
                        "C": f"{row['Option_C']} ({row['Score_C']}分)",
                        "D": f"{row['Option_D']} ({row['Score_D']}分)",
                    }
                })
        ...
        new_row = {
            ...
            "Details": json.dumps({k: int(v) for k, v in bank_scores.items()}, ensure_ascii=False),
            "Bank": ", ".join(bank_scores.keys()),
            "Answer_Details": json.dumps(detailed_answers, ensure_ascii=False),
        }
        save_result(new_row)
        st.balloons()
        
        st.divider()
        st.success(f"提交成功！您的得分为：{total_score} 分")
        st.info("请等待面试官进一步通知。")


def generate_word_report(row):
    """生成Word格式的成绩报告"""
    if not HAS_DOCX:
        return None
    ...
    # 标题
    heading = doc.add_heading('面试测评结果报告', 0)
    ...
    cells[0].text = '手机号'
    cells[1].text = str(row['Phone'])
    
    cells = table.rows[2].cells
    cells[0].text = '提交时间'
    cells[1].text = str(row['Timestamp'])
    
    cells = table.rows[3].cells
    cells[0].text = '总得分'
    cells[1].text = f"{row['Total_Score']} 分"
    
    # 得分详情
    doc.add_heading('二、得分详情', level=1)
    try:
        ...
        if isinstance(details, dict):
            ...
            for bank, score in details.items():
                row_cells = table.add_row().cells
                row_cells[0].text = str(bank)
                row_cells[1].text = str(int(score))
        else:
            doc.add_paragraph(str(row['Details']))
    except:
        doc.add_paragraph(str(row['Details']))
        
    # 答题详情
    doc.add_heading('三、答题情况详情', level=1)
    
    try:
        ans_raw = row.get('Answer_Details')
        answer_details = []
        if isinstance(ans_raw, str) and ans_raw.strip():
            answer_details = json.loads(ans_raw)
        elif isinstance(ans_raw, (list, dict)):
            answer_details = ans_raw
            
        for idx, item in enumerate(answer_details):
            p = doc.add_paragraph()
            p.add_run(f"题目 {idx + 1}: {item['Question']}").bold = True
            
            # 清理选择的选项文本中的冗余标签
            selected_text = str(item.get('Selected_Text', '')).strip()
            # 移除可能的 A. 或 A、 前缀
            selected_text = re.sub(r'^[A-D][.、]\s*', '', selected_text)
            
            doc.add_paragraph(f"选择: {item['Selected_Option']}. {selected_text}")
            doc.add_paragraph(f"得分: {int(item['Score'])}")
            
            # 显示所有选项及其分值
            doc.add_paragraph("选项详情:")
            if 'Options' in item:
                for opt, text in item['Options'].items():
                    # 清理选项文本中的冗余标签
                    clean_text = str(text).strip()
                    # 移除可能的 A. 或 A、 前缀
                    clean_text = re.sub(r'^[A-D][.、]\s*', '', clean_text)
                    doc.add_paragraph(f"  {opt}: {clean_text}")
            
            doc.add_paragraph("-" * 30)
    except Exception as e:
        doc.add_paragraph(f"解析答题详情出错: {e}")
...
def generate_word_report_all(df_res: pd.DataFrame):
    if not HAS_DOCX:
        return None
    doc = Document()
    h = doc.add_heading('面试测评成绩报表', 0)
    h.alignment = 1
    p = doc.add_paragraph(datetime.now().strftime("%Y-%m-%d %H:%M"))
    ...
    for _, r in df_res.iterrows():
        ...
        try:
            ...
            if isinstance(det_raw, dict):
                d = det_raw
            elif isinstance(det_raw, str) and det_raw.strip():
                try:
                    d = json.loads(det_raw)
                except Exception:
                    import ast
                    d = ast.literal_eval(det_raw)
            if isinstance(d, dict):
                det_text = '; '.join([f"{k}: {int(v)}" for k, v in d.items()])
            else:
                det_text = str(det_raw)
        except:
            det_text = str(det_raw)
        row_cells = table.add_row().cells
        row_cells[0].text = str(r.get('Timestamp', ''))
        row_cells[1].text = str(r.get('Name', ''))
        row_cells[2].text = str(r.get('Phone', ''))
        row_cells[3].text = str(int(r.get('Total_Score', 0)))
        row_cells[4].text = det_text
    bio = io.BytesIO()
    doc.save(bio)
    bio.seek(0)
    return bio
...
def admin_view():
    ...
    if len(df_res) > 0:
        ...
        edited_res = st.data_editor(
            ...
            column_config={
                ...
                "Answer_Details": st.column_config.TextColumn("答题详情JSON", disabled=True),
            }
        )
        ...
        if len(edited_res) != len(df_res):
            if st.button("💾 保存成绩变动", type="primary"):
                try:
                    # 确保包含 Answer_Details 列并统一为字符串
                    if "Answer_Details" not in edited_res.columns:
                        edited_res["Answer_Details"] = ""
                    edited_res["Answer_Details"] = edited_res["Answer_Details"].apply(
                        lambda v: "" if pd.isna(v) else (json.dumps(v, ensure_ascii=False) if isinstance(v, (dict, list)) else str(v))
                    )
        ...
        with col2:
            if st.button("🗑️ 清空所有记录", use_container_width=True):
                if st.session_state.get('confirm_clear') != True:
                    ...
                    try:
                        pd.DataFrame(columns=["Timestamp", "Name", "Phone", "Total_Score", "Details", "Bank", "Answer_Details"]).to_excel(RESULTS_FILE, index=False)
        ...
        with col3:
            if HAS_DOCX:
                all_report = generate_word_report_all(edited_res)
                st.download_button(
                    "📄 下载成绩报表（Word）",
                    data=all_report,
                    file_name=f"成绩报表_{datetime.now().strftime('%Y%m%d')}.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    use_container_width=True,
                )
            else:
                st.warning("未安装 python-docx 库，无法生成 Word 报表")
    else:
        st.info("暂无考试结果")
    ...
    if len(df_res) > 0:
        st.divider()
        st.subheader("📑 答题详情与报告导出")
        
        # 选择要查看的候选人
        candidate_options = [f"{row['Name']} - {row['Phone']} ({row['Timestamp']})" for _, row in df_res.iterrows()]
        selected_candidate_idx = st.selectbox("选择候选人查看详情", range(len(candidate_options)), format_func=lambda x: candidate_options[x])
        
        if selected_candidate_idx is not None:
            selected_row = df_res.iloc[selected_candidate_idx]
            
            col_d1, col_d2 = st.columns([2, 1])
            
            with col_d1:
                st.markdown(f"**姓名:** {selected_row['Name']}")
                st.markdown(f"**得分:** {selected_row['Total_Score']}")
                
                # 展示得分详情（各题库分数）
                det_raw = selected_row.get('Details')
                parsed_scores = {}
                try:
                    if isinstance(det_raw, dict):
                        parsed_scores = det_raw
                    elif isinstance(det_raw, str) and det_raw.strip():
                        try:
                            parsed_scores = json.loads(det_raw)
                        except Exception:
                            import ast
                            parsed_scores = ast.literal_eval(det_raw)
                except:
                    parsed_scores = {}
                if isinstance(parsed_scores, dict) and parsed_scores:
                    st.caption("得分详情")
                    st.dataframe(
                        pd.DataFrame([{"题库": k, "得分": int(v)} for k, v in parsed_scores.items()]),
                        use_container_width=True,
                        hide_index=True
                    )
                
                ans_raw = selected_row.get('Answer_Details')
                parsed_details = []
                try:
                    if isinstance(ans_raw, str) and ans_raw.strip():
                        try:
                            parsed_details = json.loads(ans_raw)
                        except Exception:
                            import ast
                            parsed_details = ast.literal_eval(ans_raw)
                    elif isinstance(ans_raw, (list, dict)):
                        parsed_details = ans_raw
                except:
                    parsed_details = []
                if parsed_details:
                    with st.expander("查看详细答题记录", expanded=True):
                        for idx, item in enumerate(parsed_details):
                            st.markdown(f"**{idx+1}. {item['Question']}**")
                            st.text(f"选择: {item['Selected_Option']} ({int(item['Score'])}分)")
                            st.caption(f"选项内容: {item['Selected_Text']}")
                            st.divider()
                else:
                    st.info("该记录无详细答题数据（可能是旧版本数据）")
                    
            with col_d2:
                if HAS_DOCX:
                    word_file = generate_word_report(selected_row)
                    if word_file:
                        st.download_button(
                            label="📄 下载 Word 报告",
                            data=word_file,
                            file_name=f"{selected_row['Name']}_面试测评报告.docx",
                            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                            use_container_width=True
                        )
                else:
                    st.warning("未安装 python-docx 库，无法生成 Word 报告")
